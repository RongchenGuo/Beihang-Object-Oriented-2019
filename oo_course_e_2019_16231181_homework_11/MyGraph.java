...
public class MyGraph implements Graph {
    ...
    private int getFirst() {
        return dataset.get(0).getNode(0);
    }
    ...
    public int calculateBlock() {
        ...
        HashSet<Integer> setFull = new HashSet<>(nodeCounter.keySet());
        while (set.size() < nodeCounter.keySet().size()) {
            blockCounter = blockCounter + 1;
            int tmpfNode = 0;
            int tmptNode = 0;
            int k = 0;
            for (Integer t: setFull) {
                k = k + 1;
                if (k == 1) { tmpfNode = t; }
                else if (k == 2) { tmptNode = t; }
                else { break; }
            }
            try {
                isConnected(tmpfNode, tmptNode);
            } catch (NodeIdNotFoundException e) {
                e.printStackTrace();
            }
            if (distance.containsKey(tmpfNode)) {
                set.add(tmpfNode);
                setFull.remove(tmpfNode);
                for (Integer node: distance.get(tmpfNode).keySet()) {
                    if (!node.equals(tmpfNode)) {
                        set.add(node);
                        setFull.remove(node);
                    }
                }
                ...
                set.add(tmptNode);
                setFull.remove(tmptNode);
                for (Integer node: distance.get(tmptNode).keySet()) {
                    if (!node.equals(tmptNode)) {
                        set.add(node);
                        setFull.remove(node);
                    }
                }
