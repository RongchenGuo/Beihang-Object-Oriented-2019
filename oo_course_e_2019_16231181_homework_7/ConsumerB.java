...
public class ConsumerB implements Runnable {
    ...
    public ConsumerB(PublicPool pool, PublicBoxB box) {
        this.pool = pool;
        this.box = box;
        this.floor = 1;
        this.direction = -1;
    ...
    private int closeDoor(int flag) {
        if (flag == 1) {
            ...
            TimableOutput.println(String.format("CLOSE-%d-B", floor));
        }
        return 0;
    }

    private int openDoor() {
        TimableOutput.println(String.format("OPEN-%d-B", floor));
        try {
            Thread.sleep(200);
        } catch (InterruptedException e) {
            e.printStackTrace();
    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                Thread.sleep(500);
                TimableOutput.println(String.format("ARRIVE-%d-B", floor));
            }
